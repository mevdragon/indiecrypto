// npx hardhat run scripts/orphan-tokens/orphan-simulation-9.script.ts --network hardhat
/**
 * Similar to Simulation 5 steep price slope with more refunds, and also smaller rake
 *
 * This simulates an extra large refund scenario with steep price slope
 * 1st buyer buys 400k USDT + 400k USDT + 400k USDT but refunds all but the last 400k USDT
 * 2nd buyer buys 400k USDT
 * 3rd buyer buys remaining amount
 *
 * We are left with 8.2M orphaned tokens (30% of total 27.2M tokens allocated for buyers)
 * This reflects how smaller rake can increase orphaned tokens
 *
 */
...
async function main() {
  ...
  console.log("\nP0: Buyer 1's first 400k USDT purchase");
  const amount400k = parseUnits("400000", 6);
  await buyerContracts[0].usdt.write.approve([fundraiser.address, amount400k]);
  ...
  console.log("\nP1: Buyer 1's second 400k USDT purchase");
  await buyerContracts[0].usdt.write.approve([fundraiser.address, amount400k]);
  await buyerContracts[0].fundraiser.write.buy([
    amount400k,
    buyer1.account.address,
  ...
  // P2: Buyer 1's third 400k purchase
  console.log("\nP1: Buyer 1's second 400k USDT purchase");
  await buyerContracts[0].usdt.write.approve([fundraiser.address, amount400k]);
  await buyerContracts[0].fundraiser.write.buy([
    amount400k,
  ...
  // P3: Buyer 2's 400k purchase
  console.log("\nP3: Buyer 2's 400k USDT purchase");
  await buyerContracts[1].usdt.write.approve([fundraiser.address, amount400k]);
