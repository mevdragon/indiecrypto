// npx hardhat run scripts/orphan-tokens/orphan-simulation-7.script.ts --network hardhat
/**
 *
 * This simulates a modest refund scenario with orphaned tokens
 * 1st buyer buys 200k USDT + 600k USDT but refunds first 200k USDT
 * 2nd buyer buys 200k USDT
 * 3rd buyer buys remaining amount
 *
 * We are left with 13.9M orphaned tokens (13% of total 100M tokens allocated for buyers)
 * These are stuck in the contract forever
 * This reflects the impact of refunds and how they can lead to orphaned tokens
 *
 */
...
async function main() {
  ...
  console.log("\nP0: Buyer 1's first 200k USDT purchase");
  const amount200k = parseUnits("200000", 6);
  const amount600k = parseUnits("600000", 6);
  await buyerContracts[0].usdt.write.approve([fundraiser.address, amount200k]);
  ...
  // P2: Buyer 2's 200k purchase
  console.log("\nP2: Buyer 2's 200k USDT purchase");
  await buyerContracts[1].usdt.write.approve([fundraiser.address, amount200k]);
  ...
  // P3: Buyer 3's purchase of remaining amount
  console.log("\nP3: Buyer 3's purchase of remaining amount");
  ...
  // Buyer 1's P1 redemption
  console.log("Redeeming Buyer 1's P1");
  await buyerContracts[0].fundraiser.write.redeem([buyer1Orders[1]]);
