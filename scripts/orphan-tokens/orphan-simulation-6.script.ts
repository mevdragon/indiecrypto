// npx hardhat run scripts/orphan-tokens/orphan-simulation-6.script.ts --network hardhat
/**
 * Similar to Simulation 5 steep price slope, but with extreme refunds
 *
 * This simulates an extra large refund scenario with steep price slope
 * 1st buyer buys 600k USDT + 600k USDT + 600k USDT + 200k but refunds all but the last 200k USDT
 * 2nd buyer buys 600k USDT
 * 3rd buyer buys remaining amount
 *
 * We are left with 5M orphaned tokens (12.5% of total 40M tokens allocated for buyers)
 * This reflects how just because theres more refunds doesnt mean there will be more orphaned tokens (parabola)
 *
 */
...
async function main() {
  ...
  console.log("\nP1: Buyer 1's third 600k USDT purchase");
  await buyerContracts[0].usdt.write.approve([fundraiser.address, amount600k]);
  await buyerContracts[0].fundraiser.write.buy([
    amount600k,
    buyer1.account.address,
  ...
  // P3: Buyer 1's fourth 200k purchase
  console.log("\nP3: Buyer 1's second 600k USDT purchase");
  const amount100k = parseUnits("100000", 6);
  await buyerContracts[0].usdt.write.approve([fundraiser.address, amount100k]);
  await buyerContracts[0].fundraiser.write.buy([
    amount100k,
    buyer1.account.address,
  ]);
  ...
  // Buyer 1 refunds P2
  console.log("\nBuyer 1 refunding P2");
  await buyerContracts[0].fundraiser.write.refund([buyer1Orders[2]]);
  console.log("P2 refund completed");

  // P4: Buyer 2's 600k purchase
  console.log("\nP4: Buyer 2's 600k USDT purchase");
  ...
  // P5: Buyer 3's purchase of remaining amount
  console.log("\nP5: Buyer 3's purchase of remaining amount");
  ...
  // Buyer 1's P3 redemption
  console.log("Redeeming Buyer 1's P3");
  await buyerContracts[0].fundraiser.write.redeem([buyer1Orders[3]]);
